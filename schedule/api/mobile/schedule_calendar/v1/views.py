...
from .serializers import MobileCalendarScheduleSerializer


class MobileCalendarScheduleViewSet(ReadOnlyDBActionsViewSetMixin,
    ...
    def get_object(self):
        ...
        if str(member_id) == str(self.request.user.id):
            schedule, created = Schedule.objects.get_or_create(member_id=self.request.user.id)
            return schedule
        raise Http404
