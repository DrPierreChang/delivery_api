...
test_location = get_v2_fuzzy_location()
...
class DriverLocationTestCase(APITestCase):
    ...
    def test_send_location(self):
        ...
        for i in range(2):
            resp = self.client.post('/api/mobile/drivers/v1/me/locations', data={
                'location': get_v2_fuzzy_location()
            })
    ...
    def test_post_locations_list(self):
        ...
        locations = [
            ...
            {'location': get_v2_fuzzy_location(), 'accuracy': 30},
        ]
        for locs in locations:
            resp = self.client.post('/api/mobile/drivers/v1/me/locations', data=locs)
            self.assertEqual(resp.status_code, status.HTTP_201_CREATED)
        self.assertEqual(Member.objects.get(id=self.driver.id).location.count(), 7)
    ...
    def test_location_improving(self):
        ...
        start_driver_location = {'lat': 53.902480, 'lng': 27.489963}
        in_progress_locations = [{'lat': 53.902349, 'lng': 27.488090}, {'lat': 53.902275, 'lng': 27.485708},
                                 {'lat': 53.902147, 'lng': 27.483910}, {'lat': 53.901301, 'lng': 27.481634}]
        ...
        with patch('routing.google.RadaroGoogleMapsClient.snap_to_roads') as snap_to_roads_mock:
            with patch('routing.google.RadaroGoogleMapsClient.directions') as directions_mock:
                ...
                resp = self.client.post('/api/mobile/drivers/v1/me/locations', data=dict(
    ...
    def test_path_history(self, patched_google):
        ...
        order_url = '/api/mobile/orders/v1/{}/'.format(order.id)
        self.client.force_authenticate(self.driver)

        resp = self.client.patch(order_url, data={'status': Order.IN_PROGRESS})
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        DriverLocationFactory.create_batch(size=batch_size,
                                           location=factory.LazyFunction(get_fuzzy_location),
                                           member=self.driver)
        resp = self.client.patch(order_url, data={'status': Order.DELIVERED})
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
...
class OfflineLocationsTestCase(APITestCase):
        ...
        cls.initial_locations = {
            'offline_history': [
                {'location': {'lat': 53.907600, 'lng': 27.515333},
                 'timestamp': to_timestamp(cls.fake_now.replace(microsecond=0) + timedelta(minutes=1))},
                {'location': {'lat': 53.907610, 'lng': 27.515331},
                 'timestamp': to_timestamp(cls.fake_now.replace(microsecond=0) + timedelta(minutes=2))},
                {'location': {'lat': 53.907620, 'lng': 27.515332},
                 'timestamp': to_timestamp(cls.fake_now.replace(microsecond=0) + timedelta(minutes=3))},
            ]
        }
        cls.raw_initial_locations = [
    ...
    def test_locations_save(self):
        self.client.force_authenticate(self.driver)
        resp = self.client.post('/api/mobile/drivers/v1/me/locations/', self.initial_locations)
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED)
        for loc in self.raw_initial_locations:
            self.assertTrue(DriverLocation.objects.filter(offline=True, **loc).exists())

    def test_locations_save_situations(self):
        self.client.force_authenticate(self.driver)
        resp = self.client.post('/api/mobile/drivers/v1/me/locations/', {'offline_history': [{
            'location': {'lat': 53.907600, 'lng': 27.515333}, 'timestamp': '2017-07-20T12:34:56Z'
        }]})
        self.assertContains(resp, 'Float is required', status_code=status.HTTP_400_BAD_REQUEST)
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
        resp = self.client.post('/api/mobile/drivers/v1/me/locations/', {'offline_history': [{
            'location': {'lat': 53.907600, 'lng': 27.515333}, 'timestamp': '123123123'
        }]})
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED)
        resp = self.client.post('/api/mobile/drivers/v1/me/locations/', {'offline_history': []})
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED)
    ...
    def test_invalid_locations(self):
        ...
        new_locs['offline_history'][1]['timestamp'] = to_timestamp(self.fake_now + timedelta(hours=3))

        resp = self.client.post('/api/mobile/drivers/v1/me/locations/', new_locs)
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            resp.data['errors']['offline_history'][1]['timestamp'][0]['message'],
            TimeMismatchingError.message
        )

        resp = self.client.post('/api/mobile/drivers/v1/me/locations/', new_locs['offline_history'][0])
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED)

        new_locs['offline_history'][1]['timestamp'] = to_timestamp(self.fake_now + timedelta(hours=6))
