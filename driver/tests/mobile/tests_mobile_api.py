...
class DriverTestCase(BaseDriverTestCase):
    ...
    def test_driver_skill_sets_workflow(self):
        ...
        resp = self.client.patch('/api/mobile/drivers/v1/me/', {'skill_set_ids': [self.skill_set.id]})
        self.assertEqual(len(resp.data['skill_set_ids']), 1)
        self.assertEqual(resp.data['skill_set_ids'][0], self.skill_set.id)

        resp = self.client.patch('/api/mobile/drivers/v1/me/', {'skill_set_ids': [self.skill_set_2.id]})
        self.assertEqual(len(resp.data['skill_set_ids']), 1)
        self.assertEqual(resp.data['skill_set_ids'][0], self.skill_set_2.id)

        resp = self.client.patch('/api/mobile/drivers/v1/me/', {})
        self.assertEqual(len(resp.data['skill_set_ids']), 1)
        self.assertEqual(resp.data['skill_set_ids'][0], self.skill_set_2.id)

        resp = self.client.patch('/api/mobile/drivers/v1/me/', {'skill_set_ids': []})
    ...
    def test_driver_hubs_workflow(self):
        ...
        resp = self.client.patch('/api/mobile/drivers/v1/me/', {'default_order_values': {'wayback_hub_ids': []}})
        ...
        resp = self.client.patch('/api/mobile/drivers/v1/me/', {'default_order_values': {
    ...
    def test_driver_profile(self):
        ...
        resp = self.client.get('/api/mobile/drivers/v1/me/')
        self.assertEqual(resp.status_code, status.HTTP_200_OK)

        resp = self.client.get('/api/mobile/drivers/v1/vehicle_types/')
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        ...
        resp = self.client.patch('/api/mobile/drivers/v1/me/', data)

        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertDictSubsetEqual(resp.data, data)
        self.assertTrue(isinstance(resp.data['vehicle']['capacity'], int))
    ...
    def test_upload_image(self):
        ...
        resp = self.client.patch(
            '/api/mobile/drivers/v1/me/upload_images/',
            data={'avatar': image},
            format='multipart',
        )
    ...
    def test_statistics(self):
        self.client.force_authenticate(self.driver)
        resp = self.client.get('/api/mobile/drivers/v1/me/statistics/')
        self.assertEqual(resp.status_code, status.HTTP_200_OK)

    def test_reset_charfield(self):
        self.client.force_authenticate(self.driver_2)
        resp = self.client.patch('/api/mobile/drivers/v1/me/', {'first_name': None})
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
    ...
    def test_reset_many_related_field(self):
        self.client.force_authenticate(self.driver_2)
        resp = self.client.patch('/api/mobile/drivers/v1/me/', {'skill_set_ids': None})
        self.driver_2.refresh_from_db()
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
    ...
    def test_delete_driver(self):
        driver = DriverFactory(merchant=self.merchant, work_status=WorkStatus.WORKING)

        skill_set = SkillSetFactory(merchant=self.merchant)
        driver.skill_sets.set([skill_set])

        hubs = HubFactory.create_batch(size=10, merchant=self.merchant)
        DriverHub.objects.create(driver=driver, hub=hubs[0])
        ...
        resp = self.client.delete('/api/mobile/drivers/v1/me/')
        ...
        self.assertEqual(order_1.status, Order.NOT_ASSIGNED)
        self.assertEqual(order_1.driver_id, None)
        order_2.refresh_from_db()
        self.assertEqual(order_2.status, Order.FAILED)
        self.assertEqual(order_2.driver_id, driver.id)

        event = Event.objects.filter(initiator=driver, event=Event.DELETED).last()
